...
Object-oriented language constructs reify objects. Once identified expressly, an object can be composed in other objects, received and returned by object methods, and generally manipulated like numbers, strings and other [first-class citizens](https://en.wikipedia.org/wiki/First-class_citizen) of the program, eventhough merely a construct in an unexecuted file. No such affordances are made for the representation of state, on the other hand. State is a side-kick, a property, a quality (i.e. “stateful” or “statefulness”) adjacent to objects. And unlike the objects to which they may attach, state can only be _observed_ at runtime, rather than _expressed_ directly in a unexecuted file. `bankAccount` and `withdrawalAmount` identify an objects, for example. Once identifed, they can be recieved and returned from object methods and generally manipulated like any other first-class citizen of the program, eventhough merely constructs in an unexectued file. By reifying these objects, however, their associated state is religated to object quality, hidden behind methods `checkBalance` and `get`, which are observable (i.e. callable) at runtime.
